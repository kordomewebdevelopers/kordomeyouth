<script lang="ts">
  ...
  $: action = check.id === "yes" ? "?/approve" : "?/reject";
...
<section>
  <h3>Promotions to approve</h3>

  {#each promotions as { id, views, status, file, campaign_id, metadata }}
    <div class="accord">
      <div class="inner">
        <div>{status}</div>
        ...
        <Menu
          dimension="20"
          on:click={() => {
            promotion = { id, views, file, status, campaign_id, metadata };
          }}
        />
      </div>
      ...
      <Accordion {id} block>
        <div>
          This promotion is ready for approval. Take the reload button to verify
          and approve it.
        </div>
      </Accordion>
    </div>
  {:else}
    <div>
      <Hero size="15" />
      <div class="info" style="padding: var(--gap-base);">
        All square, no promotion feedback to verify yet.
        <a href="/admin/promotions">Return back</a>
      </div>
    </div>
  {/each}
</section>
...
{#if promotion.id}
  {@const { id, file, views, status, campaign_id, metadata } = promotion}
